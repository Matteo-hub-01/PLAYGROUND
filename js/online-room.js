export class OnlineRoom{
  ...
  schedule(){if(this.closed||!this.code)return;const base=document.hidden?750:this.connected?80:250,delay=Math.min(2500,base*Math.max(1,2**this.failures));clearTimeout(this.timer);this.timer=setTimeout(()=>this.poll(),delay)}
  async poll(){if(this.busy||!this.code)return;this.busy=true;try{const result=await this.request({action:"poll",code:this.code,token:this.token});const changed=result.version!==this.version||result.connected!==this.connected||this.failures>0;this.failures=0;if(changed){this.version=result.version;this.connected=result.connected;this.onUpdate?.(result.payload,{...result,latency:this.latency,network:this.latency<90?"excellent":this.latency<180?"stable":"lent"})}}catch(error){this.failures=Math.min(6,this.failures+1);this.onUpdate?.(null,{error:this.failures>1?"Reconnexion en cours…":error.message,reconnecting:true,latency:this.latency})}finally{this.busy=false;this.schedule()}}
