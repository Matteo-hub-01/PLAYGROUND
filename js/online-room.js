export class OnlineRoom{
  constructor(game,onUpdate){this.game=game;this.onUpdate=onUpdate;this.code="";this.token="";this.role="";this.version=-1;this.connected=false;this.timer=0;this.busy=false}
  ...
  connect(info){this.code=info.code;this.token=info.token;this.role=info.role;this.version=-1;this.connected=false;clearInterval(this.timer);this.timer=setInterval(()=>this.poll(),180);this.poll();return info}
  ...
  async send(payload){if(!this.code)return;try{const result=await this.request({action:"send",code:this.code,token:this.token,payload});this.version=result.version}catch(error){this.onUpdate?.(null,{error:error.message})}}
