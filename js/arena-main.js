...
function predictGuestPlayer(player,control,dt){
  if(!player||!game.state.running||game.state.countdown>0)return;
  const direction=Number(Boolean(control.right))-Number(Boolean(control.left));if(!direction)return;
  const speed=game.config.moveSpeed*(player.traits?.speed||1);player.x=Math.max(-40,Math.min(game.config.width-player.width+40,player.x+direction*speed*dt));player.facing=direction;
}
...
ui.mode.addEventListener("change",()=>{window.Playground?.reset();online=ui.mode.value==="online";networkState.reset();if(!online)room.close();game.setMode(online?ARENA_MODES.PLAYER_VS_PLAYER:ui.mode.value);clearInputs();openFighterSelection("Mode changé : choisissez les combattants.")});
