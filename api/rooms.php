...
function readRoom(string $path):?array{if(!is_file($path))return null;$room=json_decode((string)file_get_contents($path),true);if(!is_array($room)||($room['expires']??0)<time()){@unlink($path);return null;}return $room;}
...
if($action==='create'){$game=preg_replace('/[^a-z-]/','',(string)($request['game']??''));if(!in_array($game,['pong','chess'],true))reply(['error'=>'Jeu inconnu'],400);do{$code=newCode();$path=roomPath($root,$code);}while(is_file($path));$token=bin2hex(random_bytes(16));$room=['code'=>$code,'game'=>$game,'host'=>$token,'guest'=>null,'version'=>0,'state'=>null,'input'=>null,'expires'=>time()+21600];file_put_contents($path,json_encode($room),LOCK_EX);reply(['code'=>$code,'token'=>$token,'role'=>'host','version'=>0]);}
...
if($action==='send'){$channel=$role==='host'?'state':'input';$payload=$request['payload']??null;if(strlen(json_encode($payload))>65536)reply(['error'=>'Message trop volumineux'],413);$handle=fopen($path,'c+');if(!$handle||!flock($handle,LOCK_EX))reply(['error'=>'Salon temporairement indisponible'],503);rewind($handle);$latest=json_decode((string)stream_get_contents($handle),true);if(is_array($latest))$room=$latest;$room[$channel]=$payload;$room['version']++;$room['expires']=time()+21600;ftruncate($handle,0);rewind($handle);fwrite($handle,json_encode($room));fflush($handle);flock($handle,LOCK_UN);fclose($handle);reply(['ok'=>true,'version'=>$room['version']]);}
if($action==='poll')reply(['version'=>$room['version'],'connected'=>boolval($room['guest']),'payload'=>$role==='host'?$room['input']:$room['state']]);
